...
$OutputEncoding = [Console]::OutputEncoding = [Text.UTF8Encoding]::new($false);
Set-Location -LiteralPath (Convert-Path .);
function Write-Utf8NoBom { param([string]$Path,[string]$Content,[switch]$Backup)
  ...
  try{
    ...
    if($Backup -and (Test-Path $Path)){ $t=(Get-Date).ToString(\"yyyyMMdd_HHmmss\"); Copy-Item $Path \"$Path.bak.$t\" }
...
Write-Utf8NoBom -Path $file -Content ($old+\"`nYOUR_TEXT_HERE`n\") -Backup
"'
```